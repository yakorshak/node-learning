...
function t5(a, b) {
    ...
    let differance = larger - less;
    let random = Math.round(differance * Math.random() + less)
...
function t9(a) {
    let min = a.reduce(function (previousReturnedMin, current) {
        return Math.min(previousReturnedMin, current);
    });
...
function t10(a) {
    let sum = 0;
    a.forEach((currentElement) => sum += currentElement);

    /*
        Способ 2:
        sum = a.reduce(function(total, current){
